...
if(isset($_POST['save'])){
     ...
     foreach($_FILES['files']['name'] as $key=>$val){ 
             ...
             $insert = $conn->query("update blog_one set blog='$editorContent' ,blog_pic='$fileName' where blog_id='$blogid'"); 
             if($insert){ 
       ...
       //if pic isn't being replaced
        ...
        $insert = $conn->query("update blog_one set blog='$editorContent' where blog_id='$blogid'"); 
        if($insert){ 
            ?><script>alert('Update successful')</script><?php
        }else{ 
            ?><script>alert("Sorry, there was an error uploading your file."); </script><?php
...
$data=$conn->query("select * from blog_one");
                    ...
                    <div class="blog_side">
                    <div class="img_side">
                    <i class="fa fa-trash del-one"style="color:red;font-size:18px;"></i><span><i class="fa fa-edit edit-one"style="color:rgb(0,130,189);font-size:18px;"></i></span>
